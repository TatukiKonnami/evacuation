...
public class Hash_Creation : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        using (System.IO.StreamReader tantenData = new System.IO.StreamReader("Tanten_List.txt", System.Text.Encoding.UTF8))
        {
            ...
            ////Dictionaryを作成する
            Dictionary<Tanten, List<Tanten>> interSection = new Dictionary<Tanten, List<Tanten>>();

            //Tanten型のListを宣言
            List<Tanten> roads_temporary_x2z2 = new List<Tanten>();
            List<Tanten> roads_temporary_x1z1 = new List<Tanten>();
            ...
            int x1_temporary = 0;
            int z1_temporary = 0;
            Tanten bbbbb = new Tanten(0, 0);
            Tanten aaaaa = new Tanten(0, 0);
            ...
            while (i < roads.Count)
            {
                ...
                while (it < tantens.Count)
                {
                    ...
                    if (roads[i].x1 == tantens[it].x && roads[i].z1 == tantens[it].z)
                    {
                        ...
                        bbbbb.x = x2_temporary;
                        bbbbb.z = z2_temporary;

                        if (!interSection.ContainsKey(tantens[it]))
                        {
                            interSection.Add(tantens[it], new List<Tanten>());

                        }

                        interSection[tantens[it]].Add(bbbbb);


                        //roads_temporary_x2z2.Add(new Tanten(roads[i].x2, roads[i].z2));
                        ...
                        //Debug.Log(roads_temporary_x2z2);
                    ...
                    if (roads[i].x2 == tantens[it].x && roads[i].z2 == tantens[it].z)
                    {
                        ...
                        aaaaa.x = x1_temporary;
                        aaaaa.z = z1_temporary;
                        ...
                        interSection[tantens[it]].Add(aaaaa);
